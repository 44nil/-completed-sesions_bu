...
class Session(db.Model):
    ...
    recur_group_id = db.Column(db.String(36), nullable=True, index=True)
...
def calendar_grid():
    ...
    return render_template('_calendar_grid.html', days=days, slots=slots, by_cell=by_cell)
def _make_days(week_start: datetime):
    """Pazartesi'den itibaren 7 günlük liste (datetime)."""
    return [week_start + timedelta(days=i) for i in range(7)]
